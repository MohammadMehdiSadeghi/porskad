-- ════════════════════════════════════════════════════════════════
-- Conditional Logic — شرطی‌سازی سوالات
-- هر سوال می‌تواند شرطی داشته باشد که بر اساس جواب سوال دیگری
-- نمایش داده شود یا نادیده گرفته شود.
--
-- ساختار condition:
-- {
...
begin
  ...
  loop
    ...
    if (q->>'id') is not null and (q->>'id') != 'null' and (q->>'id') != '' then
      ...
      insert into public.questions (form_id, type, title, description, required, options, position, condition)
      values (
        v_form_id,
        q->>'type',
        q->>'title',
