-- ══════════════════════════════════════════════════════════════
-- Migration: 0083_add_otp_pattern_code.sql
-- Description: افزودن کد الگوی اختصاصی پیامک OTP در آموت (پیش‌فرض: 6516)
-- ══════════════════════════════════════════════════════════════

-- ۱. ثبت کلید پیش‌فرض otp_pattern_code در system_settings
INSERT INTO public.system_settings (key, value, description, updated_at)
VALUES ('otp_pattern_code', '"6516"'::jsonb, 'کد الگوی وب‌سرویس آموت جهت ارسال سریع OTP (پیش‌فرض: 6516)', now())
ON CONFLICT (key) DO UPDATE
SET value = coalesce(public.system_settings.value, '"6516"'::jsonb),
    updated_at = now();
...
AS $$
  SELECT jsonb_strip_nulls(
    jsonb_build_object(
      ...
      'otp_pattern_code', coalesce((SELECT value FROM public.system_settings WHERE key = 'otp_pattern_code'), '"6516"'::jsonb),
