-- ══════════════════════════════════════════════════════════════
-- مایگریشن ۰۰۲۴: حالت نمایش سوالات (buttons / dropdown)
-- این مایگریشن رو حتماً اجرا کن!
-- ══════════════════════════════════════════════════════════════

-- ۱. اضافه کردن ستون display_mode
  ...
  FOR q IN SELECT value FROM jsonb_array_elements(p_questions) AS elem LOOP
    ...
    ELSE
      INSERT INTO public.questions (form_id, type, title, description, required, placeholder,
        validation, options, position, conditions, jump_actions, correct_answer, points, display_mode)
...
DECLARE
  ...
  v_fid uuid;
BEGIN
  -- پشتیبانی از هم public_id و هم uuid
