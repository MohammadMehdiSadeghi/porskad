-- ══════════════════════════════════════════════════════════════
-- Placeholder داینامیک برای سوالات فرم
-- ══════════════════════════════════════════════════════════════

-- اضافه کردن فیلد placeholder به جدول questions
...
begin
  ...
  loop
    ...
    if (q->>'id') is not null and (q->>'id') != 'null' and (q->>'id') != '' then
      ...
      insert into public.questions (form_id, type, title, description, required, placeholder, options, position, condition, conditions, jump_actions)
      values (
        v_form_id,
        q->>'type',
        q->>'title',
