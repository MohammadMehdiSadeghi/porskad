-- ══════════════════════════════════════════════════════════════
-- فیلد validation برای سوالات — اعتبارسنجی سفارشی
-- ══════════════════════════════════════════════════════════════

-- اضافه کردن فیلد validation
ALTER TABLE public.questions
  ADD COLUMN IF NOT EXISTS validation jsonb DEFAULT NULL;
...
BEGIN
  ...
  LOOP
    ...
    ELSE
      INSERT INTO public.questions (form_id, type, title, description, required, placeholder, validation, options, position, conditions, jump_actions)
      VALUES (
        v_form_id,
        q->>'type',
        q->>'title',
