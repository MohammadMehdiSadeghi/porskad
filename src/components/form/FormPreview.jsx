...
function MiniTextInput({ q }) {
  return (
    <input
      ...
      className="w-full bg-white border border-ink/20 rounded-lg px-2.5 py-1.5 text-xs font-semibold text-ink/40 placeholder:text-ink/30 text-right placeholder:text-right"
    />
  );
}
...
function MiniLongTextInput({ q }) {
  return (
    <textarea
      ...
      rows={2}
      placeholder={getPlaceholder(q)}
      className="w-full bg-white border border-ink/20 rounded-lg px-2.5 py-1.5 text-xs font-semibold text-ink/40 placeholder:text-ink/30 resize-none text-right placeholder:text-right"
    />
  );
}
  ...
  if (displayMode === "dropdown" && !isMulti) {
    return (
      <div className="w-full border border-ink/15 bg-white rounded-lg px-2 py-1.5 flex items-center justify-between">
        <span className="text-xs font-semibold text-ink/40">یک گزینه انتخاب کنید...</span>
    ...
    <div className="flex flex-col gap-1">
      {(options || []).map((opt, i) => (
        <div key={i} className="flex items-center gap-1.5 border border-ink/15 bg-white rounded-lg px-2 py-1">
          <span className={`w-4 h-4 shrink-0 flex items-center justify-center border border-ink/20 text-xs font-bold text-navy ${isMulti ? "rounded" : "rounded-full"}`}>
            {faNum(i + 1)}
          </span>
          <span className="text-xs font-semibold text-ink">{opt}</span>
        </div>
      ))}
    </div>
  );
...
function MiniYesNo() {
  return (
    <div className="grid grid-cols-2 gap-1.5">
      <div className="flex items-center justify-center py-1.5 border border-ink/15 bg-white rounded-lg text-xs font-bold text-ink">بله</div>
      <div className="flex items-center justify-center py-1.5 border border-ink/15 bg-white rounded-lg text-xs font-bold text-ink">خیر</div>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col gap-1.5 border border-ink/10 bg-bg-mint/30 rounded-lg p-2.5">
      <div className="flex items-center gap-1.5">
        <span className="w-4 h-4 flex items-center justify-center bg-navy text-white rounded-full text-xs font-black">
          {faNum(index + 1)}
        </span>
        <span className="text-xs font-bold text-ink/40 flex items-center gap-1">{(() => { const Icon = QUESTION_TYPE_ICONS[q.type]; return Icon ? <Icon size={10} /> : null; })()} {meta.label}</span>
        {q.required && <span className="text-xs text-magenta-text font-bold">*</span>}
      </div>
      <h3 className="text-xs font-black text-navy leading-4">{q.title || "سوال بدون عنوان"}</h3>
      {q.description && <p className="text-xs text-ink/40 -mt-0.5">{q.description}</p>}
  ...
  return (
    <div className="p-3 flex flex-col gap-2.5 min-h-[300px] max-h-[500px] overflow-y-auto">
      ...
      <div className="text-center mb-1">
        ...
        {form?.description && <p className="text-xs text-ink/40 mt-0.5">{form.description}</p>}
      </div>
      ...
      {/* دکمه ارسال */}
      {questions.length > 0 && (
        <div className="bg-teal text-white text-xs font-bold text-center py-1.5 rounded-lg mt-1">
          ارسال پاسخ
        </div>
      )}
    </div>
    ...
    <div className="p-3 flex flex-col gap-3 min-h-[300px] max-h-[500px] overflow-y-auto">
      ...
      {step === -1 && (
        <div className="flex flex-col items-center text-center gap-2 py-4">
          ...
          <p className="text-xs text-ink/50 leading-5 max-w-[200px]">
            {form?.welcome_message || "ممنون که وقت گذاشتی."}
          </p>
          <button onClick={() => setStep(0)} className="mt-2 bg-teal text-white text-xs font-bold px-4 py-1.5 rounded-full">
            شروع ←
          </button>
          {total > 0 && <span className="text-xs text-ink/30">{faNum(total)} سوال</span>}
        </div>
      )}
      ...
      {step >= 0 && step < total && (
        <div className="flex flex-col gap-2">
          ...
          <div className="flex items-center justify-between mt-1">
            <button onClick={() => setStep((s) => Math.max(-1, s - 1))} className="text-xs font-bold text-ink/40 hover:text-ink/60">
              ← برگشت
            </button>
            <span className="text-xs text-ink/30">{faNum(step + 1)} از {faNum(total)}</span>
            {step < total - 1 ? (
              <button onClick={() => setStep((s) => s + 1)} className="text-xs font-bold text-teal hover:text-teal-text">
                بعدی →
              </button>
            ) : (
              <button onClick={() => setStep(total)} className="text-xs font-bold text-magenta-text hover:text-magenta">
                ثبت نهایی
              </button>
            )}
          </div>
      ...
      {step >= total && total > 0 && (
        <div className="flex flex-col items-center text-center gap-2 py-4">
          ...
          <button onClick={() => setStep(-1)} className="mt-2 text-xs font-bold text-ink/40 hover:text-ink/60 flex items-center gap-1">
