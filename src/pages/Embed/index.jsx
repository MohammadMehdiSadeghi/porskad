...
// ─── مولفه‌های UI ساده (بدون وابستگی به کامپوننت‌های پنل) ───
...
// ─── انواع سوال ───
function TextInput({ type, value, onChange, autoFocus = true, onEnter }) {
  const placeholders = {
    short_text: "...",
  ...
  if (type === "long_text") {
    return (
      <textarea
        ...
        rows={4}
        autoFocus={autoFocus}
        className="w-full border-2 border-gray-200 rounded-xl px-4 py-3 text-base focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 focus:outline-none resize-y min-h-[6rem]"
  ...
  return (
    <input
      ...
      className={`w-full border-2 border-gray-200 rounded-xl px-4 py-3 text-base focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 focus:outline-none ${isLtr ? "text-left" : ""}`}
      placeholder={placeholders[type]}
    />
  );
}
    ...
    <div className="flex flex-col gap-2">
      {options.map((opt, i) => (
        <button
          ...
          className={`flex items-center gap-3 text-right w-full border-2 rounded-xl px-4 py-3 transition-all cursor-pointer ${
            value === opt
        ...
        >
          <span className={`w-8 h-8 shrink-0 flex items-center justify-center rounded-full border-2 text-sm font-bold ${
            value === opt ? "border-indigo-500 bg-indigo-500 text-white" : "border-gray-300 text-gray-600"
          }`}>{faNum(i + 1)}</span>
    ...
    <div className="grid grid-cols-2 gap-3">
      {["بله", "خیر"].map((label) => (
        <button
          key={label}
          type="button"
          onClick={() => { onChange(label); setTimeout(onEnter, 300); }}
          className={`flex flex-col items-center gap-1 border-2 rounded-xl py-5 text-lg font-bold transition-all cursor-pointer ${
            value === label
              ? label === "بله"
    ...
    <div className="flex justify-center gap-2" dir="ltr">
      {[1, 2, 3, 4, 5].map((n) => (
        <button
          key={n}
          type="button"
          className={`text-4xl cursor-pointer transition-transform hover:scale-110 ${n <= current ? "" : "opacity-30 grayscale"}`}
...
function BrandingBadge({ formId }) {
  return (
    <div className="text-center py-3 border-t border-gray-100 mt-4">
      <a
        href={`https://porscode.ir?utm_source=embed_badge&utm_form=${formId}`}
        target="_blank"
        rel="noopener noreferrer"
        className="inline-flex items-center gap-1.5 text-xs text-gray-400 hover:text-indigo-500 transition-colors"
      >
        ساخته‌شده با
        <span className="font-bold text-indigo-500">پرس‌کد</span>
      </a>
    </div>
  );
}
...
export default function EmbedForm() {
  ...
  useEffect(() => {
    async function load() {
      try {
        // استفاده از RPC برای دریافت Schema فرم
  ...
  if (loading) return <div className="min-h-screen bg-gray-50 flex items-center justify-center"><Spinner label="در حال بارگذاری..." /></div>;
  if (error) return <div className="min-h-screen bg-gray-50 flex items-center justify-center"><div className="text-center p-8"><p className="text-lg font-bold text-gray-800">{error}</p></div></div>;
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans" dir="rtl">
      ...
      <div className="w-full max-w-xl mx-auto px-4 py-3">
        <span className="text-xs font-bold text-gray-400 truncate block text-center">{schema.title}</span>
      </div>

      {/* نوار پیشرفت */}
      {step >= 0 && step < total && (
        <div className="w-full max-w-xl mx-auto px-4 pb-2">
          <ProgressBar value={step} max={visibleTotal} />
        </div>
      )}

      <main className="flex-1 flex items-start justify-center px-4 py-6">
        <div className="w-full max-w-xl bg-white rounded-2xl shadow-lg border border-gray-100 p-6 sm:p-8">
          <AnimatePresence mode="wait" custom={dir}>
            ...
            {step === -1 && (
              ...
              >
                <h1 className="text-2xl font-black text-gray-900">{schema.welcome_title}</h1>
                <p className="text-gray-600 leading-7">{schema.welcome_message}</p>
                {visibleTotal > 0 && (
                  <span className="text-xs text-gray-400">{faNum(visibleTotal)} سوال</span>
                )}
                <button onClick={goNext} disabled={visibleTotal === 0}
                  className="mt-4 bg-indigo-600 text-white px-8 py-3 rounded-xl font-bold hover:bg-indigo-700 transition-colors disabled:opacity-50">
                  شروع کنید 🚀
                </button>
              </motion.div>
            )}
            ...
            {step >= 0 && step < total && currentQuestion && (
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="text-sm font-bold text-gray-400">سوال {faNum(currentVisibleIndex)} از {faNum(visibleTotal)}</span>
                  {currentQuestion.required && <span className="text-xs font-bold text-red-400">اجباری *</span>}
                </div>
                <h2 className="text-lg font-bold text-gray-900">{currentQuestion.title}</h2>
                {currentQuestion.description && <p className="text-sm text-gray-500 -mt-2">{currentQuestion.description}</p>}

                {(currentQuestion.type === "short_text" || currentQuestion.type === "long_text" || currentQuestion.type === "email" || currentQuestion.type === "number" || currentQuestion.type === "phone_ir") && (
                  <TextInput type={currentQuestion.type} value={answers[currentQuestion.id]} onChange={setAnswer} onEnter={goNext} />
                )}
                ...
                {requiredError && (
                  <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-2 text-sm font-bold text-red-600">{requiredError}</div>
                )}

                <div className="flex items-center justify-between mt-2">
                  <button onClick={goBack} className="text-sm font-medium text-gray-500 hover:text-gray-700">برگشت</button>
                  {step < total - 1 ? (
                    <button onClick={goNext} className="bg-gray-900 text-white px-6 py-2.5 rounded-xl font-bold hover:bg-gray-800 transition-colors">بعدی ←</button>
                  ) : (
                ...
                {submitError && (
                  <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-2 text-sm font-bold text-red-600 self-end">{submitError}</div>
                )}
              </motion.div>
            )}
            ...
            {step >= total && (
              ...
              >
                ...
                <h1 className="text-2xl font-black text-gray-900">{schema.exit_title}</h1>
                <p className="text-gray-600 leading-7">{schema.exit_message}</p>
              </motion.div>
            )}
          </AnimatePresence>
