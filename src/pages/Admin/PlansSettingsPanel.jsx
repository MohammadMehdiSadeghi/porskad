...
// تم کارت استیکری هر طرح
const PLAN_CARD_THEME = {
  free: { theme: "white", rotate: "md:rotate-[-0.6deg]" },
  pro: { theme: "teal", rotate: "md:rotate-[0.5deg]" },
  enterprise: { theme: "orange", rotate: "md:rotate-[-0.5deg]" },
};
const CUSTOM_CARD = { theme: "navy", rotate: "" };
...
const labelCls = "block text-[11px] font-black text-navy dark:text-slate-200 mb-1";
...
export default function PlansSettingsPanel() {
  ...
  return (
    <div className="space-y-6">
      {/* ─── نوار ابزار: وضعیت + اکشن‌ها ─── */}
      <StickerCard theme="navy" className="w-full">
        <div className="p-4 sm:p-5 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div className="flex items-start gap-3">
            <div className="w-10 h-10 rounded-xl bg-teal/15 dark:bg-teal-950/50 text-teal dark:text-teal-300 flex items-center justify-center shrink-0">
              <Crown size={20} />
            </div>
            <div>
              <h3 className="text-sm sm:text-base font-black text-navy dark:text-slate-100">
                مدیریت اشتراک‌ها و تعرفه‌ها
              </h3>
              <p className="text-xs font-semibold text-ink-subtle dark:text-slate-400 mt-0.5 leading-relaxed">
                قیمت، سهمیه و امکانات هر طرح قابل ویرایش است. ذخیره برای کل سامانه (صفحه تعرفه‌ها و محدودیت‌ها) اعمال می‌شود.
              </p>
              <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-[10px] font-bold text-ink-subtle dark:text-slate-500 mt-2">
                ...
                {dirty && <span className="text-magenta dark:text-pink-400">● تغییرات ذخیره‌نشده</span>}
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6 xl:gap-7 items-start">
        {planIds.map((planKey, orderIdx) => {
          ...
          const cardMeta = PLAN_CARD_THEME[planKey] || CUSTOM_CARD;

          return (
            <StickerCard key={planKey} theme={cardMeta.theme} rotate={cardMeta.rotate} className="w-full">
              <div className="p-4 sm:p-5 flex flex-col gap-4">
                {/* سربرگ */}
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0 flex items-center gap-2.5">
                    <div className="w-9 h-9 rounded-xl bg-navy dark:bg-slate-100 text-white dark:text-navy flex items-center justify-center shrink-0 rotate-[-3deg]">
                      <Crown size={17} />
                    </div>
                    <div className="min-w-0">
                      ...
                      <span className="text-[10px] font-mono font-bold text-ink-subtle dark:text-slate-500" dir="ltr">
                        {planKey}
                      </span>
                    </div>
                  </div>
                ...
                {/* فلگ‌ها */}
                <div className="flex gap-4 text-xs font-bold text-navy dark:text-slate-300">
